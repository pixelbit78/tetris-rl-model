...
class TetrisAgent():
    ...
    def select_action(self, state, eps_threshold=0.1):
        ...
        if sample > eps_threshold:
            ...
            final_action = torch.tensor([[random.randrange(3)]], device=self.device, dtype=torch.long)
    ...
    def train(self):
        ...
        for num_episode in range(1, EPISODES):
            ...
            if num_episode % LOG_EVERY_EPISODE == 0:
                if loss:
                    print(f'[{num_episode}] loss: {loss:.2f} mem: {len(self.memory)} avg steps: {sum(avg_steps)/LOG_EVERY_EPISODE:.2f} eps:  {eps_threshold:.2f} max score:  {max_score} avg reward: {sum(avg_rewards)/LOG_EVERY_EPISODE:.2f} top reward: {reward_record:.2f} ')
